...
#define GPS_RX    6         //connect to TX of module 
#define GPS_TX    7         //connect to RX of module
//Serial gpsSerial(Serial1);
...
void setup()
{
  ...
  //ON  = 0x01
  //
  byte settingsArray[] = {0x03, 0xF4, 0x01, 0x80, 0x25, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //
...
//#define SIMONE_RESET
#ifdef SIMONE_RESET
  byte setRST[] = {0xB5, 0x62, 0x06, 0x04, 0x04, 0x00, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00};
  calcChecksum(&setRST[2], sizeof(setRST) - 4);
#endif
  delay(2500);
...
#ifdef SIMONE_RESET
  while (gpsSetSuccess < 3) {
    Serial.print("FULL GPS RESET");
    sendUBX(setRST, sizeof(setRST));
    gpsSetSuccess += getUBX_ACK(&setRST[2]);
    if (gpsSetSuccess == 10) gpsStatus[6] = true;
    if (gpsSetSuccess == 5 | gpsSetSuccess == 6) gpsSetSuccess -= 4;
  }
  if (gpsSetSuccess == 3) Serial.println("RESET .... booh!");
...
byte getUBX_ACK(byte *msgID) {
  ...
  if (msgID[0] == ackPacket[6] && msgID[1] == ackPacket[7] && CK_A == ackPacket[8] && CK_B == ackPacket[9]) {
    ...
    Serial.print(" ACK OK - Checksum Failure: ");
    printHex(ackPacket, sizeof(ackPacket));
    delay(1000);
    return 1;
  }
