/**************************************************************************//**
 * \brief Eeprom24XX512 library for Arduino
 * \author Copyright (C) NEAT based on 2012  Julien Le Sech - www.idreammicro.com
 * \version 1.0
 * \date 20161222
 *
...
/**************************************************************************//**
 * \fn Eeprom24XX512::Eeprom24XX512(byte deviceAddress)
 *
 * \brief Constructor.
 *
 * \param   deviceAddress   EEPROM address on TWI bus.
 ******************************************************************************/
Eeprom24XX512::Eeprom24XX512
(
    byte deviceAddress
){
...
/**************************************************************************//**
 * \fn void Eeprom24XX512::initialize()
 *
 ...
 ******************************************************************************/
void
Eeprom24XX512::initialize()
{
    Wire.begin();
}

/**************************************************************************//**
 * \fn void Eeprom24XX512::writeByte(
 * word address,
 * byte data)
 *
 * \brief Write a byte in the EEPROM memory number dependi.
 *
 ...
 ******************************************************************************/
void
Eeprom24XX512::writeByte
(
    word    address,
    byte    data
){
    Wire.beginTransmission((byte)(m_deviceAddress));
...
/**************************************************************************//**
 * \fn void Eeprom24XX512::writeBytes(
 * word     address,
 * word     length,
 * byte*    p_data)
 *
...
(
    ...
    if (pageOffset > 0)
    {
		...
		if (notAlignedLength>length)notAlignedLength=length;
...
/**************************************************************************//**
 * \fn byte Eeprom24XX512::readByte(word address)
 *
...
(
    ...
    Wire.write((int)(address & 0xFF)); // LSB
    Wire.endTransmission();
    Wire.requestFrom((byte)(m_deviceAddress), (byte)1);
    byte data = 0;
    while (!Wire.available()) ;  // wait data
...
/**************************************************************************//**
 * \fn void Eeprom24XX512::readBytes(
 * word     address,
 * word     length,
 * byte*    p_data)
 *
...
/**************************************************************************//**
 * \fn void Eeprom24XX512::writePage(
 * word     address,
 * byte     length,
 * byte*    p_data)
 *
...
/**************************************************************************//**
 * \fn void Eeprom24XX512::writeBuffer(
 * word     address,
 * byte     length,
 * byte*    p_data)
 *
...
/**************************************************************************//**
 * \fn void Eeprom24XX512::readBuffer(
 * word     address,
 * byte     length,
 * byte*    p_data)
 *
...
(
    ...
    Wire.write((int)(address & 0xFF)); // LSB
    Wire.endTransmission();
    Wire.requestFrom((byte)(m_deviceAddress ), length);
    for (byte i = 0; i < length; i++)
    {
